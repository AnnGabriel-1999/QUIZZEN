...
while($datarow = $result->fetch(PDO::FETCH_ASSOC)){
    $quiz_item = array(
        ...
        'passingrate' => $datarow['passingrate']
    );
  ...
  $quizModel->quizOwner = $_GET['owner'];
  $quizModel->capability = $_GET['capability'];
  $quizModel->passingrate = $datarow['passingrate'];
  $quizModel->filepath = $datarow['filepath'];

  if ($quizModel->addQuizInSharing()) {
...
while($datarow2 = $result2->fetch(PDO::FETCH_ASSOC)){
    $parts_item = array(
        ...
        if ($datarow3['type_id']== 1){
          //FOR MULTIPLE CHOICE
          $quizModel->quizID =  $_GET['MaxID']+1;
          $quizModel->part_id = $_GET['MaxPart']+$inc;
          ...
          array_push($quizModel->values, $datarow3['choice4']);
            ...
            if($quizModel->addQuestion()){
                                echo json_encode(
                                    array('success' => 'Question added successfully'));
                            }else{  
                                 echo json_encode(
                                    array(
                                        'message' => 'There is an error in adding questions'
