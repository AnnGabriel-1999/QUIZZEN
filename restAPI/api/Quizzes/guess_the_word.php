        ...
        if($errorCont->checkField($_POST['correct'], 'Answer', 0, 201)){
	        ...
	        if ($quiz->GenericAddQuestion()){
	        	if($quiz->GenericInsertQuestion()){
	        		...
	        		echo json_encode( array('success' => 'Question Added Successfully' ));
	        	}else{
	        		echo json_encode( array('error' => 'There is an error in adding correct answer' ));
	        	}
	        }else{
	        	echo json_encode( array('error' => 'There is an error in adding question' ));
	        }
